...
import (
	...
	snowflake "github.com/adbc-drivers/snowflake/go"
...
var drv = snowflake.NewDriver(alloc)
...
func initLoggingFromEnv(db adbc.DatabaseLogging) {
	...
	case "tempfile":
		...
		f, err := os.CreateTemp("", "adbc-snowflake.*.log")
		if err == nil {
			sink = f
		} else {
			fmt.Fprintf(os.Stderr, "[snowflake] could not open temporary log file: %v", err)
		}
	}
